...
float feedforward_linear = 0.0;   // 0-100
float feedforward_angular = 0.0;  // 0-100
...
bool goXYT(int x, int y, int t) {
  ...
  if (s > MIN_POS_RES) {
    ...
    v = pid_output + feedforward_linear;  // PID + Feedforward
    if (v > 150) v = 150;
    if (v < -150) v = -150;
  }
  ...
  if (abs(dw) > MIN_DEG_RES) {
    ...
    w = pid_output + feedforward_angular;  // PID + Feedforward
    if (dw < 0) w *= -1;
    if (w > 80) w = 80;
    if (w < -80) w = -80;
  }
...
// PID parameters - sesuaikan nilai ini melalui tuning
// float Kp_linear = 0.8, Ki_linear = 0.05, Kd_linear = 0.15;
float Kp_linear = 0.2, Ki_linear = 0.0, Kd_linear = 0.05;
float Kp_angular = 0.2, Ki_angular = 0.0, Kd_angular = 0.01;


// Kp_linear , Ki_linear , Kd_linear ,feedforward_linear , Kp_angular, Ki_angular, Kd_angular


float pid_linear(float error) {
  integral_linear += error;
  float derivative = error - last_error_linear;
  ...
  return Kp_linear * error + Ki_linear * integral_linear + Kd_linear * derivative;
}

float pid_angular(float error) {
  integral_angular += error;
  float derivative = error - last_error_angular;
  ...
  return Kp_angular * error + Ki_angular * integral_angular + Kd_angular * derivative;
}
