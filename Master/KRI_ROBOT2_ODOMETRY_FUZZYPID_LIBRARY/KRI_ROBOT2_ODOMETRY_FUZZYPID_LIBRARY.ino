...
AdaptiveFuzzyPID fuzzyLinear(1.0, 0.0001, 0.1);
AdaptiveFuzzyPID fuzzyAngular(1.0, 0.0001, 0.1);

void setup_fuzzyPID() {
  // ====== FUZZY LINEAR ======
  fuzzyLinear.setSampleTime(1);
  fuzzyLinear.setInputRange(-200, 200);
  fuzzyLinear.setOutputRange(-150, 150);
  fuzzyLinear.setMembershipFunctionInputType(Triangle);
  fuzzyLinear.setMembershipFunctionOutputType(Triangle);
  fuzzyLinear.setInferenceMode(MamdaniMaxMin);

  // ====== FUZZY ANGULAR ======
  fuzzyAngular.setSampleTime(1);
  fuzzyAngular.setInputRange(-180, 180);
  fuzzyAngular.setOutputRange(-80, 80);
  fuzzyAngular.setMembershipFunctionInputType(Triangle);
  fuzzyAngular.setMembershipFunctionOutputType(Triangle);
  fuzzyAngular.setInferenceMode(MamdaniMaxMin);
}

float pid_linear(float s) {
  return fuzzyLinear.update(0, -s);
}
float pid_angular(float a) {
  return fuzzyAngular.update(0, -a);
}
...
void updateCMPS() {
  ...
  while (GY25Serial.available()) {
    ...
    // Jika sudah selesai satu frame (baris)
    if (tmp == '\n') {
      cmps.buffer[cmps.counter] = 0;
      // Parse heading (GY-25 format: "YPR=...,Heading,...")
      float newHeading = atof(strtok(cmps.buffer + 5, ","));
      // Hitung selisih raw
      float diff = newHeading - cmps.lastHeading;
      // Normalisasi ke -180..180
      if (diff > 180.0f) diff -= 360.0f;
      if (diff < -180.0f) diff += 360.0f;
      // Tambahkan selisih ke continuous heading
      cmps.continuousHeading += diff;
      // Simpan heading terbaru
      cmps.heading = newHeading;
      cmps.lastHeading = newHeading;
      // Reset buffer
      cmps.counter = 0;
      // Debug
      // Serial.printf("H: %.2f | CH: %.2f | d: %.2f\n",
      //               cmps.heading,
      //               cmps.continuousHeading,
      //               diff);
    }
  }
}
...
bool goXYT(int x, int y, int t) {
  ...
  if (abs(dw) > MIN_DEG_RES) {
    float pid_output = fuzzyAngular.update(0, -dw);
    w = pid_output + 5.0;
    ...
    while (slaveSerial.available()) {
      ...
      if (c == '\n') {
        ...
        if (idx == 6) {
          ...
          Serial.printf("OK: %.3f %.3f %.3f || %.3f %.3f %.3f\n",
